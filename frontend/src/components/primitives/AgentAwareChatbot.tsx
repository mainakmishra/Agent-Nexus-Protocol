      ...
      >
        {isAssistant ? (
          ...
          <div className="flex w-full flex-col gap-2 items-end">
            <MessageContent className="bg-muted text-primary max-w-[85%] rounded-3xl px-5 py-2.5 whitespace-pre-wrap sm:max-w-[75%] md:max-w-[60%]">
              {message.parts
                .map((part) => (part.type === "text" ? part.text : null))
                .join("")}
            </MessageContent>
...
const LoadingMessage = memo(() => (
  <Message className="mx-auto flex w-full max-w-4xl flex-col justify-start gap-2 px-2 md:px-10">
    <div className="group flex w-full flex-col gap-0">
...
const ErrorMessage = memo(({ error }: { error: Error }) => (
  <Message className="not-prose mx-auto flex w-full max-w-4xl flex-col justify-start gap-2 px-2 md:px-10">
